...
abstract class BaseRecyclerAdapter<T : BaseModel, VM : BaseRecyclerItemViewModel<T>, DB : ViewDataBinding>(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder<T, VM, DB> {
        ...
        return ItemViewHolder(binding.root, binding, viewModel) { binding, viewModel, position ->
    ...
    class ItemViewHolder<T : BaseModel, VM : BaseRecyclerItemViewModel<T>, DB : ViewDataBinding>(
        ...
        internal fun setItem(item: T, position: Int) {
            ...
            binding.setVariable(
                BR.viewModel,
                viewModel
            ) // layout'taki viewModel variable'ın adı "viewModel" olmalı
            onItemBinding(binding, viewModel, position)
