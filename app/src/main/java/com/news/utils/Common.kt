...
fun Date.toOutputDateFormat(outputFormat: String): String {
    return try {
        val dateFormat = SimpleDateFormat(outputFormat, Locale("TR"))
...
fun <T> AppCompatActivity.startNewActivity(
    type: Class<T>,
    passItems: HashMap<String, Serializable>? = null
) {
...
fun <T> AppCompatActivity.startActivityForResult(
    toClass: Class<T>,
    passItems: HashMap<String, Serializable>?,
    requestCode: Int
) {
...
fun <T> AppCompatActivity.startActivityClearAllActivities(
    toClass: Class<T>,
    bundles: HashMap<String, Serializable>?
) {
    val intent = Intent(this, toClass)
    if (bundles != null) {
...
fun <T> AppCompatActivity.startNewActivityParcelableExtra(
    type: Class<T>,
    passItems: HashMap<String, Parcelable>? = null
) {
    val intent = Intent(this, type)
    if (passItems != null) {
        for ((key, value) in passItems) {
...
fun AppCompatActivity.getInternalStorageDirectoryFile(
    directoryName: String,
    fileName: String,
    extension: String
): File {
    val path = this.getDir(directoryName, Context.MODE_PRIVATE)
    path.mkdirs()
    return File("$path/$fileName.$extension")
}
...
fun ViewPager.setupViewPager(
    ...
    fragments: Map<String, BaseFragment<*, *>>
) {
...
fun EditText.openDateTimePickerDialog(
    dateFormat: String,
    hourFormat: String,
    onDateSet: (dateStr: String) -> Unit,
    context: Context,
    selectedDateText: String? = null,
    selectedDateFormat: String? = null
) {
    openDatePickerDialog(dateFormat, { dateStr ->
        openTimePickerDialog(hourFormat, { hourStr ->
...
fun EditText.openDatePickerDialog(
    dateFormat: String,
    onDateSet: (dateStr: String) -> Unit,
    context: Context,
    selectedDateText: String? = null,
    ...
    val successListener =
        DatePickerDialog.OnDateSetListener(function = { datePicker, year, monthOfYear, dayOfMonth ->
...
fun EditText.openTimePickerDialog(
    dateFormat: String,
    onDateSet: (dateStr: String) -> Unit,
    title: String,
    context: Context,
...
fun Bitmap.controlImageOrientation(uri: Uri, context: Context): Bitmap? {
    val ei = context.contentResolver.openInputStream(uri)?.let {
        ExifInterface(
            it
        )
    }
...
fun retriveVideoFrameFromVideo(
    videoPath: String,
    onSuccess: (b: Bitmap) -> Unit,
...
fun AppCompatActivity.openGoogleMaps(longtitude: Double, latitude: Double) {
    val gmmIntentUri =
        Uri.parse("https://www.google.com/maps/dir/?api=1&destination=$latitude,$longtitude&travelmode=driving")
...
fun WebView.showSvgFile(svgAssetPath: String) {
    var htmlTemplate =
        "<html><head></head><body><img src=\"file:///android_asset/$svgAssetPath\"></body></html>"
...
fun Context.showPopUpMenu(view: View, menuId: Int, onItemClicked: (id: Int) -> Unit) {
    ...
    try {
        val method = popupMenu.getMenu().javaClass.getDeclaredMethod(
